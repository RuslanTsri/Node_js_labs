...
describe('retryOperation', () => {
    let consoleSpy;
    // Використання lifecycle hooks та jest.spyOn для логування [cite: 355-364]
    beforeEach(() => {
    ...
    it('має успішно виконатися з першої спроби', async () => {
        // Вказуємо тип: функція, що повертає Promise з рядком
    ...
    it('має викинути помилку, якщо всі спроби вичерпано (перевірка дефолтного maxRetries = 3)', async () => {
        // І тут
        const op = jest.fn().mockRejectedValue(new Error('fatal error'));
...
describe('processInBatches', () => {
    ...
    it('має коректно обробляти масив партіями', async () => {
        const processor = jest.fn(async (batch) => batch.map(n => n * 2));
